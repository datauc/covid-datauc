shinyServer(function(input, output, session) {
  ...
  output$g_reg_nuevos_xlsx <- downloadHandler(
    filename = "reg_nuevos.xlsx",
    content = function(filename) {
      writexl::write_xlsx(f_reg_nuevos(), filename)
    },
    contentType = "application/xlsx"
  )
  
  # Out todas las regiones ----
  ...
  # Casos ranking país ----
  g_comuna_casos_ranking <- reactive({
    req(
      covid_comuna(),
      g_comuna_tasa()
    )
  ...
  # Tasas ranking país ----
  g_comuna_tasa_ranking <- reactive({
    req(
      covid_comuna(),
      g_comuna_casos_ranking()
    )
  ...
  # Output comuna ----
  output$g_comuna_int <- renderGirafe({
    girafe(
      ggobj = g_comuna(),
      width_svg = ifelse(dimension_horizontal() < 800, 10, 12), # responsividad horizontal
  ...
  # Output comuna nuevos ----
  output$g_comuna_nuevos_int <- renderGirafe({
    girafe(
      ggobj = g_comuna_nuevos(),
      width_svg = ifelse(dimension_horizontal() < 800, 10, 12), # responsividad horizontal
      height_svg = 7,
      options = list(
        opts_tooltip(use_fill = TRUE),
  ...
  # Output comuna tasa ----
  output$g_comuna_tasa_int <- renderGirafe({
    girafe(
      ggobj = g_comuna_tasa(),
      width_svg = ifelse(dimension_horizontal() < 800, 10, 12), # responsividad horizontal
      height_svg = 7,
  ...
  # Output comuna tasa ranking ----
  output$g_comuna_tasa_ranking_int <- renderGirafe({
    girafe(
      ggobj = g_comuna_tasa_ranking(),
      # width_svg = 16,
      width_svg = ifelse(dimension_horizontal() < 800, 10, 12), # responsividad horizontal
      height_svg = 7,
      options = list(
  ...
  # Output comuna casos ranking ----
  output$g_comuna_casos_ranking_int <- renderGirafe({
    girafe(
      ggobj = g_comuna_casos_ranking(),
      # width_svg = 16,
      width_svg = ifelse(dimension_horizontal() < 800, 10, 12), # responsividad horizontal
      height_svg = 7,
      options = list(
  ...
  # Datos Hospitalización de pacientes en sistema integrado ----
  ...
  # Output Hospitalización de pacientes en sistema integrado ----
  ...
  # Output Hospitalizados totales por grupo de edad y género  ----
  ...
  # Output Hospitalizados UCI por grupo de edad ----
  ...
  # Output Pacientes en UCI por grupo de edad ----
  ...
  # Datos Fallecidos por región incremental ----
  ...
  # Output Fallecidos por región incremental ----
  ...
  # Datos Casos nuevos por fecha de inicio de síntomas por comuna ----
  ...
  # Output Casos nuevos por fecha de inicio de síntomas por comuna ----
  ...
  # Datos Casos activos por fecha de inicio de síntomas y comuna ----
  ...
  # Output Casos activos por fecha de inicio de síntomas y comuna ----
  ...
  # Output  Casos por genero y grupo de edad----
